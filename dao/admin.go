...
type Admin struct {
	...
	IsDelete int `json:"is_delete" gorm:"column:is_delete" description:"删除标记"`
}
...
func (t *Admin) LoginCheck(c *gin.Context, tx *gorm.DB, param *dto.AdminLoginInput) (*Admin, error) {
	...
	if err != nil {
		return nil, errors.New("用户名对应的用户信息不存在或者已经被删除")
	}
	//判断sha256(adminInfo.salt,params.Password)是否与adminInfo.password相等
	saltPassword:=public.GenSaltPassword(adminInfo.Salt,param.PassWord)

	if adminInfo.Password!=saltPassword{
		return nil,errors.New("用户名对应的密码错误，请重新输入")
	}
